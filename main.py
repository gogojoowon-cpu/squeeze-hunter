...
DELISTED = {
            "UVXY"}
# ALL_SYMBOLS — 테마 분류 참조용만 (실제 종목 수집은 Massive API 동적 수집)
...
def sqs(m:dict)->dict:
    ...
    if m.get("macd_golden_cross", False):
        macd_score = 5.0   # 골든크로스 = 상승 전환 신호
    elif m.get("macd_dead_cross", False):
        macd_score = -3.0  # 데드크로스 = 하락 전환 패널티
    elif m.get("macd_histogram", 0) > 0:
        macd_score = 2.0   # MACD 양수 = 상승 모멘텀
...
SIC_THEME_MAP = {
    # AI/반도체
    range(3674, 3675): "🤖 AI·반도체",   # 반도체
    range(7372, 7375): "🤖 AI·반도체",   # 소프트웨어
    range(3672, 3673): "🤖 AI·반도체",   # 전자부품
    # 바이오/헬스케어
    range(2836, 2837): "💊 바이오·헬스케어",  # 제약
    range(8000, 8100): "💊 바이오·헬스케어",  # 의료서비스
    range(2830, 2836): "💊 바이오·헬스케어",  # 의약품
    range(3841, 3852): "⚕️ 의료기기·디지털헬스",  # 의료기기
    # 에너지
    range(1311, 1390): "🛢️ 에너지·원자재",   # 석유/가스
    range(1400, 1500): "🛢️ 에너지·원자재",   # 광업
    range(4911, 4940): "☀️ 신재생·수소·원자력",  # 전력
    # 금융
    range(6000, 6300): "💳 핀테크·결제",   # 금융서비스
    range(6300, 6400): "💳 핀테크·결제",   # 보험
    # 미디어/통신
    range(4800, 4900): "📱 소셜·스트리밍·이커머스",  # 통신
    range(7810, 7820): "🌐 메타버스·게임",  # 엔터테인먼트
    range(7990, 7998): "🌐 메타버스·게임",  # 게임
    # 방산
    range(3760, 3770): "🪖 방산·전쟁·지정학",  # 군수
    range(3812, 3813): "🪖 방산·전쟁·지정학",  # 방산전자
    # EV/운송
    range(3711, 3717): "⚡ EV·전기차",   # 자동차
    range(3559, 3560): "⚡ EV·전기차",   # 산업기계
}

def sic_to_theme(sic: int, name: str, price: float) -> str:
    """SIC 코드로 테마 자동 분류"""
    name_up = name.upper() if name else ""
    # 이름 키워드 우선
    if any(k in name_up for k in ["BITCOIN","CRYPTO","BLOCKCHAIN","MINING","MINER"]):
        return "₿ 암호화폐·블록체인"
    if any(k in name_up for k in ["ELECTRIC VEH","CHARGING","EV ","AUTONOMOUS"]):
        return "⚡ EV·전기차"
    if any(k in name_up for k in ["THERAPEUTICS","BIOSCIENCE","BIOTECH","GENOMIC","ONCOLOGY"]):
        return "💊 바이오·헬스케어"
    if any(k in name_up for k in ["DEFENSE","AEROSPACE","DRONE","MILITARY"]):
        return "🪖 방산·전쟁·지정학"
    if any(k in name_up for k in ["SOLAR","WIND","HYDROGEN","NUCLEAR","RENEWABLE"]):
        return "☀️ 신재생·수소·원자력"
    if any(k in name_up for k in ["ARTIFICIAL INTEL","SEMICONDUCTOR","QUANTUM"]):
        return "🤖 AI·반도체"
    # SIC 코드
    if sic:
        for rng, theme in SIC_THEME_MAP.items():
            if sic in rng:
                return theme
    # 가격 기준
    if 0 < price <= 5:
...
def fetch_massive_news_batch(api_key: str, limit: int = 1000) -> dict:
    """Massive News API
    GET /v2/reference/news
    파라미터: published_utc.gte, order=desc, sort=published_utc, limit(max 1000)
    응답: results[].title, .tickers[], .published_utc
          results[].insights[].ticker, .sentiment (positive/negative/neutral)
    """
    if not HAS_REQ or not api_key: return {}
    from datetime import datetime, timezone, timedelta
    out = {}
    cutoff = (datetime.now(timezone.utc) - timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%SZ")
    CATALYST_KW = ["fda","approval","merger","acquisition","buyout","earnings",
                   "beat","squeeze","short","contract","guidance","upgrade",
                   "clinical","trial","phase","activist","catalyst","recall",
                   "investigation","lawsuit","partnership","license"]
    try:
        r = requests.get(
        ...
        print(f"  📰 뉴스 응답: {r.status_code}")
        if r.status_code != 200:
            print(f"  ⚠️ 뉴스 오류: {r.text[:200]}")
            return {}
        ...
        for article in results:
            ...
            is_cat    = any(kw in title for kw in CATALYST_KW)
            # insights는 [{ticker, sentiment, sentiment_reasoning}]
            sent_map = {
                ins.get("ticker",""): ins.get("sentiment","neutral")
            ...
            for sym in tickers:
                ...
                if sent == "positive": out[sym]["pos"] += 1
                elif sent == "negative": out[sym]["neg"] += 1
        # 감성 점수 계산
        for sym in out:
            n = out[sym]["news_count"]
            out[sym]["sentiment_score"] = round(
                (out[sym]["pos"] - out[sym]["neg"]) / max(n, 1), 2)
        print(f"  ✅ 뉴스 커버리지: {len(out)}개 종목")
    except Exception as e:
        print(f"  ❌ 뉴스 오류: {e}")
    return out


def fetch_massive_float_batch(api_key: str) -> dict:
    """Massive Float API
    GET /stocks/vX/float
    파라미터: limit(max 5000), sort=ticker.asc, cursor(페이지네이션)
    응답: results[].ticker, .free_float(주식수), .free_float_percent(%)
    """
    if not HAS_REQ or not api_key: return {}
    ...
    while True:
        try:
            params = {"limit": 5000, "sort": "ticker.asc", "apiKey": api_key}
            if cursor:
                params["cursor"] = cursor
            r = requests.get(
                "https://api.polygon.io/stocks/vX/float",
                params=params, timeout=30)
            if r.status_code != 200:
                print(f"  ⚠️ Float {r.status_code}: {r.text[:200]}")
                break
            data    = r.json()
            ...
            print(f"  ❌ Float 오류: {e}"); break
    print(f"  ✅ Float 수집: {len(out)}개")
    return out


def fetch_massive_rsi(sym: str, api_key: str) -> float:
    """Massive RSI API
    GET /v1/indicators/rsi/{stockTicker}
    파라미터: timespan=day, window=14, series_type=close, order=desc, limit=1
    응답: results.values[0].value (RSI 값 0~100)
    """
    if not HAS_REQ or not api_key: return 50.0
    try:
        r = requests.get(
            f"https://api.polygon.io/v1/indicators/rsi/{sym}",
            params={
                "timespan": "day",
                "adjusted": "true",
                "window": 14,
...
def fetch_massive_macd(sym: str, api_key: str) -> dict:
    """Massive MACD API
    GET /v1/indicators/macd/{stockTicker}
    파라미터: timespan=day, short_window=12, long_window=26, signal_window=9
              series_type=close, order=desc, limit=2 (크로스 감지용)
    응답: results.values[0].value(MACD선), .signal(신호선), .histogram
    """
    if not HAS_REQ or not api_key: return {}
    try:
        r = requests.get(
            f"https://api.polygon.io/v1/indicators/macd/{sym}",
            params={
                "timespan": "day",
                "adjusted": "true",
                "short_window": 12,
        ...
        macd_v = float(cur.get("value", 0) or 0)
        sig_v  = float(cur.get("signal", 0) or 0)
        ...
        return {
            ...
            # 골든크로스: histogram 음→양 전환
            "golden_cross": hist_v > 0 and prev_h <= 0,
            # 데드크로스: histogram 양→음 전환
            "dead_cross": hist_v < 0 and prev_h >= 0,
        }
    except: return {}


def fetch_massive_short_interest_batch(api_key: str) -> dict:
    """Massive Short Interest API
    GET /stocks/v1/short-interest
    파라미터: settlement_date.gte, settlement_date.lte, limit(max 50000), sort
    응답: results[].ticker, .short_interest, .days_to_cover, .avg_daily_volume, .settlement_date
    """
    if not HAS_REQ or not api_key: return {}
    ...
    try:
        ...
        print(f"  📊 Short Interest 응답: {r.status_code}")
        if r.status_code != 200:
            print(f"  ⚠️ {r.text[:300]}")
            return {}
        results = r.json().get("results", [])
        print(f"  📊 Short Interest 원본: {len(results)}개")
        seen = set()
        for res in results:
            sym   = res.get("ticker", "")
            ...
            if si_sh > 0:
                out[sym] = {
                    ...
                    "dtc": round(dtc, 2),
                    "avg_vol": avg_v,
                    "si_pct": 0.0,  # float_shares로 나중에 계산
                }
        print(f"  ✅ Short Interest 수집: {len(out)}개")
    except Exception as e:
        print(f"  ❌ Short Interest 오류: {e}")
    return out


def fetch_massive_short_volume_batch(api_key: str) -> dict:
    """Massive Short Volume API
    GET /stocks/v1/short-volume
    파라미터: date(YYYY-MM-DD), limit(max 50000), sort=ticker.asc
    응답: results[].ticker, .short_volume_ratio(%), .short_volume, .total_volume, .date
    """
    if not HAS_REQ or not api_key: return {}
    from datetime import date, timedelta
    out    = {}
    ...
    try:
        ...
        print(f"  📊 Short Volume 응답: {r.status_code}")
        if r.status_code != 200:
            print(f"  ⚠️ {r.text[:300]}")
            return {}
        results = r.json().get("results", [])
        print(f"  📊 Short Volume 원본: {len(results)}개")
        for res in results:
            sym   = res.get("ticker", "")
        ...
        print(f"  ✅ Short Volume 수집: {len(out)}개")
    except Exception as e:
        print(f"  ❌ Short Volume 오류: {e}")
    return out


def fetch_polygon_short_interest(syms: list, api_key: str) -> dict:
    """하위 호환용 래퍼"""
    return fetch_massive_short_interest_batch(api_key)
...
def load_real(syms:list)->dict:
    ...
    # Massive API로 전체 상장 종목 자동 수집
    print("📋 Massive API에서 전체 상장 종목 수집 중...")
    ...
    ticker_sics  = {t["symbol"]: t.get("sic", 0) for t in all_ticker_info}
    print(f"✅ 전체 수집: {len(dynamic_syms)}개")
    if not dynamic_syms:
        print("❌ 티커 수집 실패 — API 응답 확인 필요")
        return {}
    ...
    for sym, snap in snaps.items():
        try:
            ...
            # RSI: Massive API 우선, 실패 시 aggs 계산값
            rsi_api = fetch_massive_rsi(sym, POLY_KEY)
            rsi_v   = rsi_api if rsi_api != 50.0 else agg.get("rsi14", 50.0)
            ...
            # 테마 자동 분류 (SIC 코드 기반)
            sic_code = ticker_sics.get(sym, 0)
            theme = SYM_THEME.get(sym, sic_to_theme(sic_code, name, price))
...
def enrich(syms:list, state:dict):
    ...
    # Short Interest 배치 수집 (전체 한번에)
    si_data = fetch_massive_short_interest_batch(POLY_KEY)

    # Short Volume 배치 수집 (전체 한번에)
    sv_data = fetch_massive_short_volume_batch(POLY_KEY)
    ...
    # Float API로 실제 플로트 수집
    print("📊 Float 데이터 수집...")
    float_data = fetch_massive_float_batch(POLY_KEY)
    for sym, fd in float_data.items():
        if sym in state:
            state[sym]["float_shares"] = fd["free_float"]
            state[sym]["free_float_pct"] = fd["free_float_pct"]
            # SI% 재계산 (실제 플로트로)
            if sym in si_data and fd["free_float"] > 0:
                si_sh = si_data[sym].get("si_shares", 0)
                state[sym]["si_pct"] = round(si_sh / fd["free_float"] * 100, 2)
            r = sqs(state[sym]); state[sym].update(r)

    # MACD 수집 (상위 SQS 종목 상위 200개만 — rate limit)
    top_syms = sorted(
        [s for s in syms if s in state],
        key=lambda x: state[x].get("score", 0), reverse=True
    )[:200]
    print(f"📊 MACD 수집 (상위 {len(top_syms)}개)...")
    macd_ok = 0
    for sym in top_syms:
        try:
            macd = fetch_massive_macd(sym, POLY_KEY)
            if macd:
                state[sym]["macd_golden_cross"] = macd.get("golden_cross", False)
                state[sym]["macd_dead_cross"]   = macd.get("dead_cross", False)
                state[sym]["macd_histogram"]    = macd.get("histogram", 0)
                state[sym]["macd_value"]        = macd.get("macd", 0)
                state[sym]["macd_signal"]       = macd.get("signal", 0)
                r = sqs(state[sym]); state[sym].update(r)
                macd_ok += 1
            time.sleep(0.15)
        except: pass
    ...
    # 뉴스 API로 카탈리스트 + 감성 업데이트
    print("📰 뉴스 데이터 수집...")
    news_data = fetch_massive_news_batch(POLY_KEY, limit=1000)
    news_cnt = 0
    for sym, nd in news_data.items():
        if sym in state:
            state[sym]["has_catalyst"] = nd["has_catalyst"]
            # 뉴스 감성 점수 반영
            sent_adj = nd["sentiment_sum"] / max(nd["news_count"], 1)
            state[sym]["news_sentiment"] = round(sent_adj, 2)
            state[sym]["news_count"] = nd["news_count"]
            state[sym]["latest_news"] = nd["news_titles"][:3]
            r = sqs(state[sym]); state[sym].update(r)
            news_cnt += 1

    enriched = len([s for s in syms if s in si_data])
    float_cnt = len([s for s in syms if s in float_data])
    print(f"✅ 보강 완료: SI%={enriched}개 Float={float_cnt}개 MACD={macd_ok}개 뉴스={news_cnt}개")
...
def init():
    ...
    # Massive API로 전체 종목 동적 수집
    real = load_real([])
    if len(real) < 10:
        print("❌ 동적 수집 실패 — API 키/플랜 확인 필요")
        _ready = True
        _enrich_done = True
        return
...
def stock_news(symbol: str):
    """종목별 최신 뉴스"""
    d = _st.get(symbol.upper())
    if not d: return {"news": [], "symbol": symbol}
    return {"symbol": symbol, "news": d.get("latest_news",[]),
